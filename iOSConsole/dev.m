...
void TranferAppForInstallOnDevice(char *udid, char *app_path)
{
    ...
    if (device)
    {
        CFStringRef path = CFStringCreateWithBytes(kCFAllocatorDefault, (UInt8 *)app_path,
            strlen(app_path), kCFStringEncodingUTF8, false);
...
void socket_callback(CFSocketRef s, CFSocketCallBackType callbackType, CFDataRef address,
...
{
    CFURLRef plist_url = CFURLCreateCopyAppendingPathComponent(NULL, disk_app_url,
        CFSTR("Info.plist"), false);
    CFReadStreamRef plist_stream = CFReadStreamCreateWithFile(NULL, plist_url);
    CFReadStreamOpen(plist_stream);
    CFPropertyListRef plist = CFPropertyListCreateWithStream(NULL, plist_stream, 0,
...
void StartDebuggingAndDetach(char *udid, char *app_path)
{
    ...
    if (device)
    {
        CFStringRef bundleId = CFStringCreateWithBytes(kCFAllocatorDefault, (UInt8 *)app_path,
            strlen(app_path), kCFStringEncodingUTF8, false);
        CFURLRef relative_url = CFURLCreateWithFileSystemPath(NULL, bundleId, kCFURLPOSIXPathStyle,
            false);
        ...
        CFSocketRef fdvendor = CFSocketCreate(NULL, AF_UNIX, 0, 0, kCFSocketAcceptCallBack,
        ...
        CFStringRef device_container_path = CFURLCopyFileSystemPath(device_container_url,
        ...
        CFStringRef disk_container_path = CFURLCopyFileSystemPath(disk_container_url,
            kCFURLPOSIXPathStyle);
        CFStringFindAndReplace(cmds, CFSTR("{disk_container}"), disk_container_path, range, 0);

        CFDataRef cmds_data =
            CFStringCreateExternalRepresentation(NULL, cmds, kCFStringEncodingASCII, 0);
