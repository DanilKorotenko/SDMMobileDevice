...
void ServiceSocketCallback(CFSocketRef s, CFSocketCallBackType callbackType, CFDataRef address,
...
void CreateLocalSocket(char *udid, struct SDM_MD_Service_Identifiers service)
{
    ...
    if (serviceCon)
    {
        ...
        CFSocketRef serviceSock = CFSocketCreate(NULL, AF_UNIX, 0x0, 0x0, kCFSocketAcceptCallBack,
            &ServiceSocketCallback, &context);
...
void PerformService(char *udid, char *service, ...)
{
    if ((udid && strlen(udid) == 0x28) && (service))
    {
        ...
        for (index = 0x0; index < SDM_MD_Service_Count; index++)
        {
            ...
            if ((strncmp(service, ident, strlen(ident)) == 0x0 && strlen(service) == strlen(ident)) ||
                (strncmp(service, shorthand, strlen(shorthand)) == 0x0 &&
                    strlen(service) == strlen(shorthand)))
            {
                break;
            }
        }
