...
void RunQueryOnDevice(SDMMD_AMDeviceRef device, char *domain, char *key, sdmmd_return_t result)
{
    if (SDM_MD_CallSuccessful(result))
    {
        ...
        {
            domainString = CFStringCreateWithCString(kCFAllocatorDefault, domain,
    ...
    if ((udid && strlen(udid) == 0x28) && key)
    {
        ...
        if (device)
        {
            ...
            if (SDM_MD_CallSuccessful(result))
            {
                ...
                if (SDM_MD_CallSuccessful(result))
                {
                    if (strncmp(domain, kAllDomains, strlen(kAllDomains)) == 0x0 &&
                        strncmp(key, kAllKeys, strlen(kAllKeys)) == 0x0)
                    {
                        for (uint32_t i = 0x0; i < SDM_MD_Domain_Count; i++)
                        {
                            ...
                            {
                                RunQueryOnDevice(device, SDMMDKnownDomain[i].domain,
                                    SDMMDKnownDomain[i].keys[j], result);
                            }
                            printf("\n");
                        }
                    }
